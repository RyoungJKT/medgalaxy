...
With the server up: `node tools/verify.mjs --eval "window.__tour.seek(4)" --shot r7-06-peak-spine` and `node tools/verify.mjs --mobile --eval "window.__tour.seek(4)" --shot r7-m-06-peak-spine` (pause index 4 is the peak; confirm against the pause order in `buildTourPauses`: rules, hivSurge, hivFade, detonation, peak, finale). If the seek needs the landing dismissed and the film finished, use the same prelude as Task 2 Step 13. Read both PNGs: the second line sits under the headline at the caption's second-line size, does not collide with the rail or the sparkline, and on the phone wraps to at most three balanced lines with no single orphaned word on the last line (plan-owner amendment 2026-09-11: the sentence is the user's verbatim wording and stays; the caption card's mobile width cannot hold 79 characters in two lines at its second-line size without shrinking the type below the data line, so the criterion is balance, not line count. Add `textWrap: 'balance'` to the caption lines in `TimeCaption` (`src/components/ui/TimeRail.jsx`) on mobile only; balance redistributes breaks and never changes a line count, and desktop lines are `nowrap` so they are untouched. Cut a mobile frame of every tour pause, rules through finale, and confirm each caption keeps its line count).
