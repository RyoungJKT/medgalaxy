...
- **One series re-backfilled, and the pairs beside it.** PubMed changed its automatic term mapping for Colorectal Cancer between the two snapshots, which left that disease's frozen 1990-2014 years answering a narrower query than its refreshed 2015-2024 years and made the seam between them read as a research surge that never happened. Its 1990-2014 years were re-queried under the current mapping the same day, so the whole series describes one query. Its 13 connection pairs were left behind by the same mapping change and re-queried on 2026-09-11 as well (`19,931` to `28,584` for breast cancer, and so on for the other twelve), because a doubled total can never trip the `sharedPapers <= min(endpoint papers)` check the weekly reconciliation had been relying on. That gap is now closed in the pipeline: a total that moves more than 25% in one run re-queries that disease's pairs on its own (`MAPPING_JUMP`, `scripts/refresh_pubmed.py`), which on the 2026-09-11 diff flags colorectal cancer and nothing else. The top-7-per-node layout set is identical before and after the re-query (619 edges, none added, none removed), so no node seat moved. It is the only series that has been rewritten, and the methodology panel's pipeline section says all of this on screen.
